  ...
  options = Object.assign(
    {
      ...
      headersPrefix: "X-RateLimit", //Prefix for custom rate limit headers
      draft_polli_ratelimit_headers: false, //Support for the new RateLimit standardization headers
      skipFailedRequests: false, // Do not count failed requests (status >= 400)
      skipSuccessfulRequests: false, // Do not count successful requests (status < 400)
      skipStatusCodes: [], // Do not count request finished with status code from the list
          ...
          Promise.resolve(maxResult)
            .then((max) => {
              ...
              if (options.headers && !res.headersSent) {
                res.setHeader(options.headersPrefix + "-Limit", max);
                res.setHeader(
                  options.headersPrefix + "-Remaining",
              ...
              if (
                ...
                options.skipStatusCodes.length > 0
              ) {
                ...
                if (options.skipStatusCodes.length > 0) {
                  res.on("finish", function () {
